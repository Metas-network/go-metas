...
import (
	...
	"github.com/Metas-network/lachesis-base/common/bigendian"
...
func (s *Store) GetRules() metas.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &metas.Rules{}).(*metas.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg metas.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() metas.Genesis {
	meatadata := s.GetMetadata()
	return metas.Genesis{
