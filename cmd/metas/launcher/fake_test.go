...
import (
	...
	"github.com/Metas-network/go-metas/inter/validatorpk"
)

func TestFakeNetFlag_NonValidator(t *testing.T) {
	// Start a metas console, make sure it's cleaned up and terminate the console
	cli := exec(t,
		"--fakenet", "0/3",
	...
	waitForEndpoint(t, filepath.Join(cli.Datadir, "metas.ipc"), 60*time.Second)

	// Verify the actual welcome message to the required template
	cli.Expect(`
Welcome to the Lachesis JavaScript console!

instance: go-metas/v{{version}}/{{goos}}-{{goarch}}/{{gover}}
coinbase: {{.Coinbase}}
at block: 1 ({{niltime}})
 datadir: {{.Datadir}}
 modules: {{apis}}
