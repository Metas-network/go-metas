...
import (
	...
	"github.com/Metas-network/go-metas/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "metas-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "metas-test" in exec().
	reexec.Register("metas-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "metas". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("metas-test", args...)
